// First-run setup wizard (`forseti --wizard`): a small ncurses flow that
// asks the handful of decisions that genuinely vary per user, writes a
// forseti.conf into the chosen data directory, creates that directory, and
...
_print_next_steps :: proc(st: ^Wiz_State, conf_path: string) {
	...
	// forseti-gui auth args, matching the RPC auth chosen in the wizard.
	auth := st.rpc_cookie \
	...
	fmt.printfln("      ./forseti --datadir=%s              # foreground, logs to terminal", dd)
	fmt.printfln("      ./forseti --datadir=%s --gui        # desktop dashboard window", dd)
	...
	fmt.println()
	fmt.println("  WATCH IT FROM ANOTHER COMPUTER:")
	fmt.println("  RPC listens on localhost only, so forward the port over SSH. Run these")
	fmt.println("  on the OTHER computer (it also needs forseti-gui built):")
	fmt.println()
	fmt.printfln("      ssh -f -N -L %d:localhost:%d <user>@<this-server>   # open the tunnel", port, port)
	if st.rpc_cookie {
		fmt.printfln("      scp <user>@<this-server>:%s/.cookie /tmp/forseti.cookie", dd)
		fmt.printfln("      ./forseti-gui --connect=127.0.0.1:%d --cookie=/tmp/forseti.cookie", port)
		fmt.println()
		fmt.println("  The cookie is regenerated on every node restart, so you must re-copy it")
		fmt.println("  after each restart. To avoid that, set rpcuser/rpcpassword in the conf")
		fmt.println("  and connect with --rpcuser/--rpcpassword instead — no cookie to manage.")
	} else {
		fmt.printfln("      ./forseti-gui --connect=127.0.0.1:%d --rpcuser=%s --rpcpassword=%s", port, st.rpc_user, st.rpc_pass)
		fmt.println("      (add --tui for a terminal dashboard)")
	}
	fmt.println()
	fmt.println("  SKIP THE SSH TUNNEL (trusted LAN only — RPC is unencrypted HTTP and can")
	fmt.println("  control the node; never expose it on a public/internet-facing server):")
	fmt.println("  add to the conf, open the firewall for the port, then connect directly:")
	fmt.println("      rpcbind = 0.0.0.0")
	fmt.println("      rpcallowip = <client-ip>/32")
	if st.rpc_cookie {
		fmt.println("      rpcuser = <user>              # cookie auth can't cross the network")
		fmt.println("      rpcpassword = <password>")
	}
	fmt.printfln("      -> ./forseti-gui --connect=<this-server-ip>:%d --rpcuser=<u> --rpcpassword=<p>", port)
	fmt.println()
}
...
_frame_open :: proc(step, total: int, title, prompt: string, rows: int) -> Frame {
	...
	_st(0, 1, "Forseti  Setup", _pair(D_TOPBAR) | nc.A_BOLD)
...
_write_config :: proc(st: ^Wiz_State) -> (string, bool) {
	...
	strings.write_string(&b, "# Generated by forseti --wizard. Edit freely; see\n")
	strings.write_string(&b, "# contrib/forseti.conf.sample for every supported key.\n\n")
...
_default_datadir :: proc() -> string {
	...
	return fmt.tprintf("%s/.forseti", home)
}
