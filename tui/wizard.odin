...
_print_next_steps :: proc(st: ^Wiz_State, conf_path: string) {
	...
	fmt.printfln("      ./btcnode-gui --connect=127.0.0.1:%d %s", port, auth)
	...
	fmt.println("  on the OTHER computer (it also needs btcnode-gui built):")
	fmt.println()
	fmt.printfln("      ssh -f -N -L %d:localhost:%d <user>@<this-server>   # open the tunnel", port, port)
	if st.rpc_cookie {
		fmt.printfln("      scp <user>@<this-server>:%s/.cookie /tmp/btcnode.cookie", dd)
		fmt.printfln("      ./btcnode-gui --connect=127.0.0.1:%d --cookie=/tmp/btcnode.cookie", port)
		fmt.println()
		...
		fmt.printfln("      ./btcnode-gui --connect=127.0.0.1:%d --rpcuser=%s --rpcpassword=%s", port, st.rpc_user, st.rpc_pass)
	...
	fmt.printfln("      -> ./btcnode-gui --connect=<this-server-ip>:%d --rpcuser=<u> --rpcpassword=<p>", port)
	fmt.println()
}
