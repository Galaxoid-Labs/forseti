...
make test         # Run all 155 tests (9 packages)
...
- `mempool/` — Fee rates, relay policy, validation pipeline (4 files)
...
- **UTXO cache**: Write-back with Dirty/Fresh flags. Flushed every 5000 blocks during sync and at shutdown.
- **Sync**: Headers-first (getheaders/headers), then block download (getdata with Witness_Block, 16 blocks in flight).
